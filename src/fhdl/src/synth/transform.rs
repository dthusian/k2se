...
use crate::synth::ir::{IRModule, IRModuleInst, IRValue, IRStmt, IRTriggerStmt, IRWireMemDecl};
...
pub fn transform_modules(modules: &[(Module, Span)], builtins: &HashMap<String, Box<dyn BuiltinFunction>>) -> (Vec<IRModule>, Vec<CerrSpan>) {
    ...
    Expr::BinaryOps { car, cdr } => {
      ...
      cdr.iter()
        .for_each(|(op, expr)| {
          ...
          resolved_arg = Some(transform_single_binary_op(state, *op, resolved_arg.take().unwrap(), irv, span));
        });
      (resolved_arg.unwrap(), true)
    }
    
    Expr::FnCall { args, func } => {
      let args = args.iter().map(|expr| transform_expr(state, expr, span).0).collect::<Vec<_>>();
      if let Some(func_box) = state.global.builtins.get(func) {
        if args.len() != func_box.arg_ty().len() {
          state.global.errors.push(Cerr::WrongNumberOfFunctionArgs(func_box.arg_ty().len()).with(span))
        }
        func_box.arg_ty()
          .iter()
          .zip(args.iter())
          ...
          .for_each(|(i, (a1, a2))| {
            ...
            match a1 {
              FunctionArgReq::Any => {}
              FunctionArgReq::NetType(ty) => {
                match a2 {
                  IRValue::Net(net) => {
                    // unwrap: IRValue nets should always be validated
                    let ty2 = state.ir_objects.get(net).unwrap().ty;
                    if &ty2 != ty {
                      state.global.errors.push(Cerr::TypeErrArgMismatch(i, func.clone(), TypeError {
                        src_ty: format!("{:?}", ty2),
                        dst_ty: format!("{:?}", ty),
                      }).with(span));
                    }
              ...
              FunctionArgReq::String => {
                if !matches!(a2, &IRValue::Str(_)) {
                  state.global.errors.push(Cerr::ExpectedString(i, func.clone()).with(span))
                }
              }
            }
          });
        ...
        state.global.errors.push(Cerr::UnknownFunction(func.clone()).with(span));
        (IRValue::Net("$error".into()), false)
      }
    }
    
    Expr::Literal { val } => {
      (IRValue::Lit(*val), false)
    }
    
    Expr::StringLiteral { str } => {
      (IRValue::Str(str.clone()), false)
    }
  }
}

fn transform_expr_and_assign_to(state: &mut ModuleValidationState, expr: &Expr, span: Span, assign_to: String) {
  let (result, is_anon) = transform_expr(state, expr, span);
  let ty = state.ir_objects[&assign_to].ty;
  match get_type(state, &result) {
    Ok(ty2) => {
      if ty != ty2 {
        state.global.errors.push(Cerr::TypeErrorGeneric(TypeError {
          src_ty: format!("{:?}", ty2),
          dst_ty: format!("{:?}", ty),
        }).with(span))
      }
    ...
    Err(err) => {
      state.global.errors.push(err.with(span))
    }
  }
  if !is_anon {
    state.stmts.push(IRStmt {
...
fn transform_single_binary_op(state: &mut ModuleValidationState, op: BinaryOp, a: IRValue, b: IRValue, span: Span) -> IRValue {
  ...
  if ty1 == NetType::Mixed && ty2 == NetType::Mixed && op != BinaryOp::Add && op != BinaryOp::Sub {
    state.global.errors.push(Cerr::InvalidOpOnMixedNets(op).with(span))
  }
...
fn get_type(state: &mut ModuleValidationState, irv: &IRValue) -> Result<NetType, Cerr> {
  Ok(match irv {
    IRValue::Net(name) => state.ir_objects.get(name).map(|v| v.ty).unwrap_or(NetType::Single),
