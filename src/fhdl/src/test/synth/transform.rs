...
impl BuiltinFunction for TestFunction {
  ...
  fn synthesize(&self, _: &[(NetID, NetID)], _: (NetID, NetID)) {
    panic!("not implemented")
  }

  fn constant_fold(&self, _: &[i32]) -> Option<i32> {
    None
  }
}
...
fn test_builtins() -> HashMap<String, Box<dyn BuiltinFunction>> {
  ...
  map.insert("test1".into(), Box::new(TestFunction { arg_ty: FunctionArgReq::NetType(NetType::Single) }));
  map.insert("test2".into(), Box::new(TestFunction { arg_ty: FunctionArgReq::NetType(NetType::Mixed) }));
  map.insert("test3".into(), Box::new(TestFunction { arg_ty: FunctionArgReq::SingleOrLit }));
  map.insert("test4".into(), Box::new(TestFunction { arg_ty: FunctionArgReq::String }));
  map
}
    ...
    (Module {
      ...
      stmts: vec![
        (Stmt::ModuleInst {
          module: "target_module".into(),
          args: vec![
            Expr::BinaryOps { car: Box::new(Expr::Literal { val: 2 }), cdr: vec![(BinaryOp::Add, Expr::Literal { val: 2 })] },
            Expr::BinaryOps { car: Box::new(Expr::Literal { val: 2 }), cdr: vec![(BinaryOp::Add, Expr::Literal { val: 2 })] },
          ],
        }, ds),
      ],
...
pub fn transform_err_arg_type_error() {
  ...
  let errs = transform_modules(&ast, &test_builtins()).1;
  let expected = vec![
    Cerr::TypeErrArgMismatch(0, "test1".into(), TypeError {
      src_ty: "Mixed".into(),
      dst_ty: "Single".into(),
    }).with(ds),
    Cerr::TypeErrArgMismatch(0, "test1".into(), TypeError {
    ...
    Cerr::TypeErrArgMismatch(0, "test2".into(), TypeError {
      src_ty: "Single".into(),
      dst_ty: "Mixed".into(),
    }).with(ds),
    Cerr::TypeErrArgMismatch(0, "test2".into(), TypeError {
    ...
    Cerr::TypeErrArgMismatch(0, "test3".into(), TypeError {
      src_ty: "Mixed".into(),
      dst_ty: "Single".into(),
    }).with(ds),
  ];
...
pub fn transform_err_unknown_func() {
    ...
    (Module {
      ...
      stmts: vec![
        (Stmt::WireDecl {
          name: "w1".into(),
          signal_class: NetType::Single,
          expr: Some(Expr::FnCall { func: "not_exist".into(), args: vec![] }),
        }, ds)
      ],
    }, ds)
  ];
  let errs = transform_modules(&ast, &test_builtins()).1;
  let expected = vec![
    Cerr::UnknownFunction("not_exist".into()).with(ds)
  ];
  assert_eq!(errs, expected);
}
...
pub fn transform_err_wrong_number_of_args() {
  ...
  let errs = transform_modules(&ast, &test_builtins()).1;
  let expected = vec![
    Cerr::WrongNumberOfFunctionArgs(1).with(ds)
  ];
  assert_eq!(errs, expected);
}
...
pub fn transform_err_expected_string() {
  ...
  let errs = transform_modules(&ast, &test_builtins()).1;
  let expected = vec![
    Cerr::ExpectedString(0, "test4".into()).with(ds)
  ];
  assert_eq!(errs, expected);
}
